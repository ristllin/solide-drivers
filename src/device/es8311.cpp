...
#include <string.h>
#include "es8311.h"
// Nimbus/solide adaptation: this is Espressif's Apache-2.0 ES8311 driver with
// ONLY its I2C transport swapped from the IDF legacy i2c_master API to Arduino
// Wire, so it shares the one I2C bus owner (solide::i2cEnsureBegun) with the
// capacitive touch controller. The register sequence + clock table are unchanged.
#include <Arduino.h>
#include <Wire.h>
#include "esp_log.h"
#include "esp_err.h"
...
es8311_handle_t es8311_create(const uint16_t dev_addr)
{
...
// (The upstream es8311_codec_init() demo helper - which used the EXAMPLE_* macros
// and ESP_ERROR_CHECK, i.e. panic-on-any-I2C-hiccup - is removed. The solide audio
// layer drives es8311_create()/es8311_init() itself with graceful failure paths.)
